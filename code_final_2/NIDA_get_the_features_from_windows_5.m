...
load('C:\Users\aungkon\Desktop\jhu\code\code_final_2\data_NIDA\window_store.mat');
...
%     mi = min(y_temp);
    ...
    y_cell{1} = y_temp;
    x_cell{1} = x_temp;
    mdl_cell{1} = @(beta,x) (((beta(1)/(taur-taud))*exp(-x*taud))-((beta(1)/(taur-taud))*exp(-x*taur(1))));
    beta0 = [100];
    ...
    plot(x,y,'b');
    plot(x,act_fit,'g');
    plot(x,y_cocaine_fit,'r');
    title(label(m));
    pause(2);
    close(figure1);

end
save(['C:\Users\aungkon\Desktop\jhu\code\code_final_2\data_NIDA\feature.mat'],'feature','label','subject');
